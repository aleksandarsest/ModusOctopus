<template>
  ...
  >
    <!-- Background decoration: technical grid, only shown when projects exist -->
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card title, derived from the simulation brief -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                <div class="modal-label">Simulation brief</div>
            ...
            <div class="modal-actions">
              ...
              >
                ...
                <span class="btn-icon">◆</span>
                <span class="btn-text">Open report</span>
              </button>
            </div>
            <!-- Replay limitation hint -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3, Run simulation, and Step 5, Deep interaction, can only be used while the simulation is running.</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Expanded state: derive height from card count
  ...
  // Height = rows * card height + gaps + a small bottom buffer
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Center each row based on its actual card count
    ...
    // Expand downward with spacing below the title
    ...
    const x = offset * 35
    // Keep the stack close to the title with some spacing
...
// Progress class by simulation rounds
...
// Generate a title from the simulation brief
...
// Format simulation_id for display
...
// Truncate file name while keeping extension
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Track the latest target state even during animation
        pendingState = shouldExpand
        
        // Clear any prior debounce timer
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // If an animation is in progress, record state and exit
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Confirm the pending state is still current
          ...
          // Release the lock after the animation finishes and check for queued updates
          setTimeout(() => {
            isAnimating = false
            
            // If a new state is queued, apply it next
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Small delay to avoid a rapid flip-flop
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Tighten the bottom margin so expansion only triggers after a real scroll
...
// Reload when returning to the home route
...
onMounted(async () => {
  // Wait for the DOM before loading data
  await nextTick()
  await loadHistory()
  
  // Initialize the observer after render
  setTimeout(() => {
    initObserver()
  }, 100)
})

// Refresh on activation when keep-alive is in play
onActivated(() => {
  loadHistory()
})

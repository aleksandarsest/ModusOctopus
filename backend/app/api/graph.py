...
def generate_ontology():
    ...
    try:
        ...
        if not document_texts:
            ...
            return jsonify({
                "success": False,
                "error": "No documents could be processed successfully. Please check the file formats."
            }), 400
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "This project does not have an ontology yet. Call /ontology/generate first."
            }), 400
        
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph build is already in progress. To force a rebuild, send force: true.",
        ...
        return jsonify({
            "success": False,
            "error": f"Failed to start graph build: {str(e)}",
            "traceback": traceback.format_exc()
        }), 500

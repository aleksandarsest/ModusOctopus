...
Step 2: Zep entity reading/filtering and OASIS simulation preparation/runtime.
"""
...
logger = get_logger('modusoctopus.api.simulation')
...
def create_simulation():
    ...
    请求（JSON）：
        {
            ...
            "graph_id": "modusoctopus_xxxx",    // 可选，如不提供则从project获取
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "This project does not have a graph yet. Call /api/graph/build first."
            }), 400
...
def prepare_simulation():
    ...
    返回：
        {
            ...
            "data": {
                ...
                "message": "Preparation started | existing preparation already available",
    ...
    try:
        ...
        if not force_regenerate:
            ...
            logger.debug(f"Prepare check result: is_prepared={is_prepared}, prepare_info={prepare_info}")
            if is_prepared:
...
def get_prepare_status():
    ...
    try:
                ...
                if is_prepared:
                    return jsonify({
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": "Task completed (existing preparation detected)",
                            "already_prepared": True,
                            "prepare_info": prepare_info
                        }
                    })
        ...
        logger.error(f"Failed to fetch task status: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": "Simulation config does not exist yet. Call /prepare first."
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "Config file does not exist yet. Call /prepare first."
            }), 404
        ...
        if not os.path.exists(db_path):
            return jsonify({
                ...
                "data": {
                    ...
                    "count": 0,
                    "posts": [],
                    "message": "Database not found. The simulation may not have started yet."
                }
            })
...
def interview_agent():
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "The simulation environment is not running or has already been closed. Make sure the simulation completed and is waiting for commands."
            }), 400
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"Interview item {i+1} has an invalid platform. Expected 'twitter' or 'reddit'."
                }), 400
...
def interview_all_agents():
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": f"Timed out while waiting for global interview responses: {str(e)}"
        }), 504
